...
static OSStatus playbackCallback(void *inRefCon, 
	...
	for (int i=0; i < ioData->mNumberBuffers; i++) { 
        ...
        if (!audioProcessor.enablePlayback) {
...
-(void)processBuffer: (AudioBufferList*) audioBufferList
{
    ...
    Float32 decibels = kDecibelOffset; // When we have no signal we'll leave this on the lowest setting
    ...
    Float32 peakValue = kDecibelOffset; // We'll end up storing the peak value here
    
    // loop over every packet
    for (int nb = 0; nb < (audioBufferList->mBuffers[0].mDataByteSize / 2); nb++) {

        Float32 absoluteValueOfSampleAmplitude = abs(editBuffer[nb]);
...
//        // Begin low-pass filter
        ...
        Float32 sampleDB = 20.0f * log10f(amplitudeToConvertToDB) + kDecibelOffset;
...
+ (AudioProcessor *)sharedAudioProcessor {
    static AudioProcessor *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[AudioProcessor alloc] init];
    });
